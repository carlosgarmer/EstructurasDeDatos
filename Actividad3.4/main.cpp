...
void insertarIp(vector<string> valor, vector< vector <string> >  &bitacoraIps){
    ...
    for( int i = 0; i < bitacoraIps.size() ; i ++ ){
        if (bitacoraIps[i][0] == valor[0]){
            bitacoraIps[i][1] = to_string(stoi(bitacoraIps[i][1]) +1 );
            anadido = true;
            // sort(bitacoraIps);
        } 
    }
    if (!anadido){
        bitacoraIps.push_back(valor);
    }
}
...
void top5(BinarySearchTree<string> bitacoraArbol){
    ...
    Node<string>*  temp;
    for( int i = 0 ; i <5 ; i++ ){
        temp = bitacoraArbol.maximum_element(bitacoraArbol.root);
        cout << temp->data << " Accesos: " << temp->key <<endl;
        bitacoraArbol.delete_node(temp->key);
    }
}
...
int main(){
  ...
  cout << "Listo!" << endl;
  bitacoraIps = soloIp(bitacoraAcomodada);
  bitacoraArbol = arbol(bitacoraIps);
  top5(bitacoraArbol);
  return 0;
}